...
func NewZoneStore(db *sql.DB) *ZoneStore {
	s := ZoneStore{
		relationalDB: db,
		query: `
		SELECT z."ID", z."name", z."description", z."area", u."ID", u."name", COALESCE(s."ID", '00000000-0000-0000-0000-000000000000'), COALESCE(s."name",''), COALESCE(s."commonNameNL",''), COALESCE(s."commonNameEN", '')
		FROM "zone" z
...
func (s *ZoneStore) process(rows *sql.Rows, err error) ([]models.Zone, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&zoneID, &zoneName, &zoneDescription, &zoneArea, &user.ID, &user.Name, &species.ID, &species.Name, &species.CommonNameNL, &species.CommonNameEN); err != nil {
