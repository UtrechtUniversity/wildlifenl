...
func NewZoneStore(db *sql.DB) *ZoneStore {
	s := ZoneStore{
		relationalDB: db,
		query: `
		SELECT z."ID", z."deactivated", z."created", z."name", z."definition", u."ID", u."name", COALESCE(s."ID", '00000000-0000-0000-0000-000000000000'), COALESCE(s."name",''), COALESCE(s."commonName",'')
		FROM "zone" z
...
func (s *ZoneStore) process(rows *sql.Rows, err error) ([]models.Zone, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&zoneID, &zoneDeactivated, &zoneCreated, &zoneName, &zoneDefinition, &user.ID, &user.Name, &species.ID, &species.Name, &species.CommonName); err != nil {
...
func (s *ZoneStore) Add(userID string, zone *models.ZoneRecord) (*models.Zone, error) {
	query := `
		INSERT INTO "zone"("name", "definition", "userID") VALUES($1, $2, $3)
		RETURNING "ID"
	`
	var id string
	row := s.relationalDB.QueryRow(query, zone.Name, zone.Definition, userID)
