...
func (s *InteractionStore) process(rows *sql.Rows, err error) ([]models.Interaction, error) {
	...
	for rows.Next() {
		...
		if q.ID != "00000000-0000-0000-0000-000000000000" {
			q.InteractionType = i.Type
			i.Questionnaire = &q
		}
...
func (s *InteractionStore) Add(userID string, interaction *models.InteractionRecord) (*models.Interaction, error) {
	...
	*/
	...
	case 3:
		query = `
			INSERT INTO "collisionReport" ("interactionID", "estimatedDamage", "severity") VALUES ($1, $2, $3)
		`
		if _, err := s.relationalDB.Exec(query, id, interaction.ReportOfCollision.EstimatedDamage, interaction.ReportOfCollision.Severity); err != nil {
			return nil, err
		}
